...
DB_ADMIN_USER="$(/usr/share/google/get_metadata_value attributes/db-admin-user || echo 'postgres')"
...
if [[ -n "${DB_ADMIN_PASSWORD_URI}" ]]; then
  ...
  readonly DB_ADMIN_PASSWORD='not_a_good_password'
fi
...
<configuration>
  <property>
    ...
    <value>jdbc:postgresql://localhost:${METASTORE_PROXY_PORT}/${METASTORE_DB}</value>
...
function initialize_metastore_db() {
  ...
  export PGPASSWORD="${DB_HIVE_PASSWORD}" 
  if ! psql -U "${DB_HIVE_USER}" -d ${METASTORE_DB} -h 127.0.0.1 -c "\l" ; then
    export PGPASSWORD="${DB_ADMIN_PASSWORD}"
    psql -U "${DB_ADMIN_USER}" -h 127.0.0.1 -c "CREATE USER ${DB_HIVE_USER} WITH PASSWORD '${DB_HIVE_PASSWORD}';"
    psql -U "${DB_ADMIN_USER}" -h 127.0.0.1 -c "CREATE DATABASE ${METASTORE_DB};"
    psql -U "${DB_ADMIN_USER}" -h 127.0.0.1 -c "GRANT CONNECT ON DATABASE ${METASTORE_DB} TO ${DB_HIVE_USER};"
    /usr/lib/hive/bin/schematool -dbType postgres -initSchema || err 'Failed to set postgres schema.'
  fi
}
